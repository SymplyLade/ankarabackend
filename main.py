...
"""
backend/main.py - Cloud-ready AnkaraPrint RAG API
"""
import os
from fastapi import FastAPI, UploadFile, File, HTTPException
from fastapi.middleware.cors import CORSMiddleware
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print("RAG failed to load:", e)
        rag_system = None  # App still runs even if RAG fails
...
app = FastAPI(
    title="AnkaraPrint RAG API",
    description="Cloud-ready Simplified API",
    version="2.0.0",
    lifespan=lifespan
)
...
@app.get("/")
async def root():
    return {"status": "ok", "service": "AnkaraPrint RAG API"}


@app.get("/status")
def status():
    return {"ready": rag_system is not None}
...
async def chat(message: ChatMessage):
    ...
    if rag_system is None:
        rag_system = create_default_rag()  # lazy load
